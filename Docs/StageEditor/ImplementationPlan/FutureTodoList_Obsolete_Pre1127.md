...
| **H-006** | **External TriggerZone UX 设计** | 🔴 **待设计** | 当前用户体验差，需要重新设计 | 本文档 |
...
### H-006: External TriggerZone UX 设计（高优先级）

**状态**：🔴 待设计

**问题背景：**
当前 External TriggerZone（外部触发区域）的用户体验非常差，需要重新设计。

**当前问题：**

1. **配置复杂**
   - 用户需要手动在场景中放置 `UStageTriggerZoneComponent`
   - 需要手动设置 `ZoneType`（LoadZone/ActivateZone）
   - 需要手动将 Component 添加到 Stage 的 `ExternalTriggerZones` 数组
   - 需要手动绑定到目标 Stage

2. **引用方式不直观**
   - Stage 通过 `TSoftObjectPtr<UStageTriggerZoneComponent>` 引用外部 Zone
   - 但 Component 没有独立 Actor，很难在场景中选中
   - 也无法在 Content Browser 中看到

3. **与 Built-in Zone 的关系不清晰**
   - Built-in Zone 和 External Zone 是组合使用的（不是互斥）
   - 但 UI 上没有清晰表达这个关系
   - 用户可能误以为二选一

4. **调试困难**
   - 运行时难以看到哪些 External Zone 被注册
   - Zone 的绑定状态不直观

**设计目标：**

1. **简化配置流程** - 减少手动步骤，提供引导式配置
2. **可视化引用关系** - 清晰展示 Stage 与 Zone 的绑定关系
3. **支持快速预览** - 在 Details 面板中展示已注册的 Zone 列表
4. **改善运行时调试** - 在 Debug HUD 中显示 Zone 状态

**可能的解决方案（待讨论）：**

| 方案 | 描述 | 优点 | 缺点 |
|------|------|------|------|
| A. Actor 包装 | 创建 `AStageTriggerZoneActor` 包装 Component | 可独立选中，支持蓝图 | 多一层包装 |
| B. 拖放绑定 | Details 面板中拖放 Actor 到数组 | 符合 UE 习惯 | 仍需手动操作 |
| C. 选择器按钮 | Details 面板中点击按钮选择场景中的 Zone | 更直观的 UI 流程 | 需要自定义 UI |
| D. 自动发现 | Stage 自动发现同 DataLayer 下的 Zone | 最自动化 | 可能不够灵活 |

**待确认问题：**

1. External Zone 的典型使用场景是什么？（复杂地形、多入口、不规则区域？）
2. 是否需要支持一个 Zone 绑定多个 Stage？
3. Zone 配置是否需要在 Stage Details 中集中管理？

**依赖项：**
- 需要先完成 M-003 Debug HUD 工具，用于验证 Zone 状态
...
| 2025-11-27 | ✅ 添加 TriggerActorTag 过滤功能到 UStageTriggerZoneComponent |
| 2025-11-27 | 📝 添加 M-003 Stage Debug HUD 工具需求 |
| 2025-11-27 | 🐛 修复玩家出生点在 TriggerZone 内时 Stage 状态不触发的问题（PlayerStart 检测方案）|
| 2025-11-27 | ✨ 添加 bDisableBuiltInZones 时可配置 InitialStageState（默认 Active）|
| 2025-11-27 | 🔄 TriggerActorTag → TriggerActorTags (TArray<FName>)，支持多标签过滤 |
| 2025-11-27 | 🐛 修复 LoadZoneExtent/ActivateZoneExtent 在 Details 面板修改后不生效的问题 |
| 2025-11-27 | 🔴 添加 H-006 External TriggerZone UX 设计（高优先级）|

---

## 中优先级 (Medium Priority) - 续
...
```
【简洁模式】
Stage_1: Active | DL: Loaded

【详细模式】
Stage_1 (ID:1)
├─ State: Active (Locked)
├─ StageDataLayer: Loaded
├─ Acts: [0✓, 1✓, 2]  (✓=激活)
├─ LoadZone: 2 actors
...
| 问题 | 决策 | 理由 |
|------|------|------|
| 显示位置 | 屏幕角落 + 可配置 | 不遮挡游戏画面，适应不同用户习惯 |
| 显示模式 | 简洁/详细可切换 | 平衡信息量和屏幕占用 |
| Actor 数量显示 | 显示 Zone 内 Actor 数 | 便于理解引用计数机制 |
| 配置存储 | Developer Settings | 统一管理，Project Settings → Plugins → Stage Editor |
| HUD 绘制方式 | AHUD::DrawHUD Canvas | 灵活控制布局，支持位置配置 |
| 编辑器模式 | 暂不支持（低优先级） | 先满足运行时调试需求 |
...
│  ├─ bEnableDebugHUD: bool                                   │
│  ├─ DisplayPosition: EStageDebugPosition (枚举/自定义)       │
│  ├─ CustomOffset: FVector2D                                 │
│  ├─ bDetailedMode: bool                                     │
│  └─ 注册到 Project Settings → Plugins → Stage Editor        │
│                                                             │
│  AStageDebugHUD : AHUD                                      │
│  ├─ DrawHUD() override                                      │
│  │   ├─ 遍历 StageManagerSubsystem->GetAllStages()          │
│  │   ├─ 根据 Settings 决定位置和模式                         │
│  │   └─ Canvas->DrawText() 绘制状态信息                      │
│  └─ 通过 GameMode 或手动设置为当前 HUD                       │
│                                                             │
│  控制台命令                                                  │
│  └─ Stage.Debug 0/1 → 修改 Settings->bEnableDebugHUD        │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

**实现步骤：**
1. 创建 `UStageDebugSettings` (UDeveloperSettings)
2. 创建 `EStageDebugPosition` 枚举（TopLeft/TopRight/BottomLeft/BottomRight/Custom）
3. 创建 `AStageDebugHUD` : AHUD，实现 DrawHUD()
4. 注册控制台命令 `Stage.Debug`
5. 在 Details 面板中暴露开关（可选，通过 Settings 已经可以配置）
